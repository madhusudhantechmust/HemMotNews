...
/*----------------------------------------------------------------------------------
	...
	Change Log:
----------------------------------------------------------------------------------*/
	if (!uid_.Get()) throw "bad UID for page.";
	...
	iMargins->GetMargins (
					&insideMargin, &topMargin, &outsideMargin, &bottomMargin);
	...
	columnCount_ = iColumns->GetNumberColumns ();
	gutterWidth_ = iColumns->GetGutter ();
...
void Basic_Page::PasteUp_(PlaceAdPanelDisplayAd* ad) throw (PMString)
{
